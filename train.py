...
if __name__ == "__main__":
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                if coverage_hist is None:
                    ...
                    coverage_hist_norm = (
                        torch.FloatTensor(coverage_hist_norm).to(device).detach()
                    )
                    (
                        action,
                        logprob,
            ...
            for start in range(0, args.num_envs, envsperbatch):
                ...
                if coverage_hist is None:
                    ...
                    coverage_loss = torch.mean(
                        torch.min(
                            torch.mean(coverage_hist_norm, 0), torch.mean(new_a_prob, 0)
                        )
                    )
                    loss = (
                        pg_loss
    ...
    torch.save(agent, f"models/ppo_optimized_{args.env_id}.pt")
    envs.close()
    if writer is not None:
        writer.close()
